# GTFS RT aggregation
# Run with: $ Rscript 00_fetch_rt/gtfs_rt_gather_TCBarreiro.R
...
# Carris Municipal
folder_path <- "data/barreiro_20260511_20260515/updates"
output_folder <- "data/barreiro_20260511_20260515"
stop_times <- "data/barreiro_20260511_20260515/stop_times.txt"
...
process_json <- function(data, filename, RECORDS_LIST, agency_id = NULL) {
  ...
  for (e in data$results$vehicles) {
    vehicle <- e
    ...
    # If previous has same coordinates, skip (to avoid duplicates)
    ...
    if (nrow(RECORDS_LIST[[trip_id_day]]) > 0 && RECORDS_LIST[[trip_id_day]] |>
      slice_tail(n = 1) |>
      pull(latitude) == latitude &&
      RECORDS_LIST[[trip_id_day]] |>
      slice_tail(n = 1) |>
      pull(longitude) == longitude) 
    {
      #message("Skipping duplicate coordinates for trip_id: ", trip_id, " on day: ", day)
      next
    }
...
mi_ma <- get_trip_stops(trips_stops, "DUPE_1-QVBB-TERM_0_DUPE_45_0437")
print(mi_ma[1])
print(mi_ma[2])
