# GTFS RT aggregation
# Run with: $ Rscript 00_fetch_rt/gtfs_rt_gather_CarrisMetropolitana_R.R
...
output_folder <- "data/cmet_20260413_220260430_business_a3"
stop_times <- "data/cmet_20260413_220260430_business/stop_times.txt"
AGENCY_ID = "43" # None 
...
process_stop_times <- function(file_path) {
  ...
  # Remove GTFS feed id from trip_id if present (e.g., "[43]trip_id" -> "trip_id")
  df$trip_id <- gsub("^\\[[^]]*\\]\\s*", "", df$trip_id)
  df$stop_sequence <- as.integer(df$stop_sequence)
  df %>%
    group_by(trip_id) %>%
    summarise(
      min = min(stop_sequence, na.rm = TRUE), 
      min_stop_id = stop_id[which.min(stop_sequence)],
      max = max(stop_sequence, na.rm = TRUE), 
      max_stop_id = stop_id[which.max(stop_sequence)],
      .groups = "drop"
    )
}
...
get_trip_stops <- function(trips_stops, trip_id) {
  ...
  if (nrow(filtered) > 0) {
    c(as.integer(filtered$min_stop_id[[1]]), as.integer(filtered$max_stop_id[[1]]))
  } else {
    ...
    purrr::map_dfr(function(trip_df) {
      ...
      return(trip_df |> mutate(
        ...
        & # Remove first and last stop (as bus tends to stop there for a while)
          stop_id != trip_trips_stops$min_stop_id[[1]] &
          stop_id != trip_trips_stops$max_stop_id[[1]]
      ) |> select(-geometry))
    })
}
...
mi_ma <- get_trip_stops(trips_stops, "1612_0_1_2000_2029_1_1")
print(mi_ma[1]) # 172486
print(mi_ma[2]) # 050418
